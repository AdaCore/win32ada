...
package body Win32.Winnt is
    ...
    function MAKELANGID (P, S: WORD) return WORD is
      use Interfaces;
    begin
      return WORD(Unsigned_32(P) or Interfaces.Shift_Left(Unsigned_32(S), 10));
    end MAKELANGID;
    ...
    function MAKELCID (Lgid, Srtid: WORD) return DWORD is
      use Interfaces;
    begin
      return DWORD(Shift_Left(Unsigned_32(Srtid), 16) or Unsigned_32(Lgid));
    end MAKELCID;
    ...
    function DECREF(X: BYTE) return BYTE is
        use Interfaces;
    begin
      return BYTE((Shift_Right(Unsigned_32(X), N_TSHIFT) and not N_BTMASK) or
